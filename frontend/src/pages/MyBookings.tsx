...
import { ApiBooking, ApiPassenger } from '@/services/api/types';
...
const MyBookings = () => {
  ...
  const { data: bookings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      return await api.bookings.getByUserId(user.id);
    },
    enabled: !!user,
  });
  ...
  const handleSendEmail = async (bookingId: string) => {
    toast.error('Email functionality is temporarily unavailable. Please print your ticket instead.');
  };
        ...
        <div className="container py-8">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {bookings.map((booking: ApiBooking) => (
                <div key={booking.id} className="bg-card rounded-2xl border border-border overflow-hidden">
                  <div className="p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                      {/* Company Info */}
                      <div className="flex items-center gap-4 lg:w-48">
                        <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center text-2xl text-primary font-bold">
                          {booking.trip?.company?.logoUrl || '🚌'}
                        </div>
                        <div>
                          <p className="font-semibold">{booking.trip?.company?.name || 'Unknown Company'}</p>
                          <p className="text-sm text-muted-foreground capitalize">{booking.trip?.bus?.busType || 'Standard'}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-medium">{booking.trip?.route?.originCity?.name}</span>
                          <ArrowRight className="h-4 w-4 text-muted-foreground" />
                          <MapPin className="h-4 w-4 text-accent" />
                          <span className="font-medium">{booking.trip?.route?.destinationCity?.name}</span>
                        </div>
                    ...
                    {booking.passengers && booking.passengers.length > 0 && (
                      <details className="mt-4">
                        <summary className="text-sm text-muted-foreground cursor-pointer hover:text-foreground outline-none">
