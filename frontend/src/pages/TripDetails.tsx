...
// Generate seats based on backend data
const generateSeats = (totalSeats: number, bookedSeats: string[] = []): Seat[] => {
  const seats: Seat[] = [];
  const cols = 4;
  const rows = Math.ceil(totalSeats / cols);
  
  // Normalize seat numbers (e.g., "1" -> "01") to match backend and frontend
  const bookedSet = new Set(bookedSeats.map(s => s.padStart(2, '0')));

  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      const seatNum = row * cols + col + 1;
      if (seatNum <= totalSeats) {
        const numberStr = seatNum.toString().padStart(2, '0');
        seats.push({
          id: `seat-${seatNum}`,
          number: numberStr,
          row,
          column: col,
          isAvailable: !bookedSet.has(numberStr),
          isSelected: false,
...
const TripDetails = () => {
  ...
  const { data: trip, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!id) return null;
      return await api.trips.getById(id);
    },
    enabled: !!id,
  });

  useEffect(() => {
    if (trip?.bus?.totalSeats) {
      setSeats(generateSeats(trip.bus.totalSeats, trip.bookedSeats));
    }
  }, [trip]);
  ...
  useEffect(() => {
    ...
    if (intent && trip && user) {
      try {
        ...
        if (data.tripId === id) {
          if (data.selectedSeats) {
            ...
            setSeats(prev => prev.map(s => ({
              ...s,
              isSelected: data.selectedSeats.includes(s.id)
            })));
          }
          if (data.passengers) {
        ...
        console.error('Failed to restore booking intent', e);
      }
    }
  }, [trip, user, id]);
        ...
        .then((data) => {
          if (data) {
            setPassengers(prev => {
              ...
              const updated = [...prev];
              if (!updated[0].fullName && data.fullName) {
                updated[0] = { ...updated[0], fullName: data.fullName };
              }
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-muted-foreground">
                        ...
                        <span>•</span>
                        <span>{(trip.bus?.company?.totalTrips || 0).toLocaleString()} trips</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{trip.route?.originCity?.name}</p>
                      <p className="text-sm text-muted-foreground">{format(new Date(trip.departureTime), 'p')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{format(new Date(trip.departureTime), 'PPP')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{trip.route?.destinationCity?.name}</p>
                      <p className="text-sm text-muted-foreground">{format(new Date(trip.arrivalTime), 'p')}</p>
                    </div>
                  </div>
                </div>
              </div>
