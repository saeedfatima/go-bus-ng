...
import { ApiTrip } from '@/services/api';
...
const TripCard = ({ trip }: TripCardProps) => {
  const departureTime = format(new Date(trip.departureTime), 'p');
  const arrivalTime = format(new Date(trip.arrivalTime), 'p');
  const company = trip.company || trip.route?.company;
      ...
      <div className="p-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">{trip.route?.originCity?.name}</p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">{trip.route?.destinationCity?.name}</p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4 pt-4 border-t border-border">
          <span className={cn(
            ...
            busTypeStyles[trip.bus?.busType || 'standard'] || busTypeStyles.standard
          )}>
