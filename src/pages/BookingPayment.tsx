...
const BookingPayment = () => {
    ...
    if (reference && isPaystackAvailable) {
      const handlePaystackCallback = async () => {
        ...
        try {
          ...
          if (result && (result.status === 'success' || result.status === 'confirmed')) {
            // Backend should have updated the status, but we'll force confirm in frontend state too
  ...
  }, [id, isPaystackAvailable, navigate, queryClient, verifyPayment]);
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-card rounded-2xl border border-border p-6">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center text-xl text-primary font-bold">
                  {booking.trip?.company?.logoUrl || '🚌'}
                </div>
                <div>
                  <p className="font-medium">{booking.trip?.company?.name}</p>
                  <p className="text-sm text-muted-foreground capitalize">{booking.trip?.bus?.busType} Bus</p>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-border">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{booking.trip?.route?.originCity?.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{booking.trip?.route?.destinationCity?.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {booking.trip?.departureTime && (
                        <>
                          {format(new Date(booking.trip.departureTime), 'PPP')} at{' '}
          ...
          <div className="bg-card rounded-2xl border border-border p-6 mt-6">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between font-semibold text-lg pt-3 border-t border-border">
                <span>Total</span>
                <span className="text-primary">₦{Number(booking.totalAmount).toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button
                ...
                disabled={confirmPayment.isPending || isProcessing || isInitializing || !isPaystackAvailable}
              >
